function searchingGetHub(q) {
	...
	return fetch(`https://api.github.com/search/repositories?q=${q}+in:name`)
		...
		.then(data => {
			...
			// console.log(`результат первичной функции`, searchResult.slice(0, 5))
...
let searchingGetHubDebounced = debounce(searchingGetHub, 300)

// async function dom() {
// 	const res = await searchingGetHubDebounced('dom2').then((result) => {return result})
// 	console.log(res)
// }
...
async function createResultList() {
	...
	// searchingGetHubDebounced(searchInput.value)
}
